...
void Sms4ExtendKey(unsigned int *subkey, const unsigned char *key)
{
    ...
    // 将 128bit的字符串密钥 转化为 密钥MK = (MK0,MK1, MK2, MK3), 字符串长度为8bit

    for (i = 0; i < SMS4_KEY_LENGTH/4; i ++)
        wkey[i] ^= sms4_fk[i];
    // (K0,K1,K2,K3)=(MK0⊕FK0 ,MK1⊕FK1,MK2 ⊕FK 2 ,MK 3 ⊕ FK 3 )
                           ...
                           unsigned char *output )
{
    ...
    {
        //
        ulbuf[i+4] = Sms4F(ulbuf[i], ulbuf[i+1], ulbuf[i+2], ulbuf[i+3], sk[i]);
// #ifdef _DEBUG
                    ...
                    unsigned char *output )
{
    ...
    if( mode == SM4_ENCRYPT )
    {
        //input: plain, output: cipher
        while( length > 0 )
        {
            for( i = 0; i < 16; i++ )
                output[i] = (unsigned char)( input[i] ^ iv[i] );

            Sms4Encrypt(output,output,ctx->sk);
            memcpy( iv, output, 16 );
    ...
    else /* SM4_DECRYPT */
    {
        ...
        while( length > 0 )
        {
            ...
            Sms4Decrypt(output,input,ctx->sk);
